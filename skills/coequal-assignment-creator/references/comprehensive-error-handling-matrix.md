...
| Rubric mapping | CoEqual changes a value on readback | Rubric accuracy | L2 | Stop before creation and ask. | Accept changed value. |
...
| Final QA | Any source/rubric field unverified | Accuracy | L2 | Ask user or re-verify read-only. | Click Create assignment. |
| Final QA | User has not confirmed creation | External side effect | L2 | Ask final confirmation. | Click Create assignment. |
...
7. Record decision.
8. Ask user if the issue affects Canvas integrity, source truth, rubric truth, privacy, or final creation.
